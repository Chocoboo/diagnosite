...
choc.timeScroll = null;  //挂载整屏切换动画的实例

choc.currentStep = "step1";

choc.init = function(){

	choc.resize(); //设置每一屏的高度和top值

	choc.events(); //配置事件

	choc.configIntAnimate();//配置导航条的动画

	choc.button3D(".start",".state1",".state2",0.3);
	choc.button3D(".button1",".state1",".state2",0.3);
	choc.button3D(".button2",".state1",".state2",0.3);

	//设置每一屏中img的百分比

	choc.imgWidth($(".scene img"));

	choc.configTimeScroll(); //配置横屏切换的动画
...
$(document).ready( choc.init );

//配置事件
choc.events = function(){
	

	choc.nav(); //执行导航条的鼠标移入移除的动画
	...
	$(window).bind("scroll",choc.scrollStatus);
	...
	$(window).bind("mouseup",choc.mouseupFn)
	...
	function mousewheelFn(ev,direction){
		...
		if( direction<1 ){  //向下滚动
			//console.log("next");
			choc.changeStep("next");
		}else{  //向上滚动
			console.log("prev");
			choc.changeStep("prev");
		};
		clearTimeout(timer);
	...
	$(window).resize( choc.resize );


};
//当mouseup的时候，让当前这一屏到达某个状态
choc.mouseupFn = function(){
	//在滚动过程中计算出一个比例
	var scale = choc.scale();
	//得到当前页面到达的某个时间点
	var times = scale * choc.timeScroll.totalDuration();

	//获取到上一个状态和下一个状态
	var prevStep = choc.timeScroll.getLabelBefore(times);
	var nextStep = choc.timeScroll.getLabelAfter(times);

	//获取到上一个状态的时间和下一个状态的时间
	var prevTime = choc.timeScroll.getLabelTime(prevStep);
	var nextTime = choc.timeScroll.getLabelTime(nextStep);
	...
	*/
	...
	choc.timeScroll.tweenTo(step);
	//-----------------当松开鼠标时候，控制滚动条到达某个状态计算出的距离-----------------------
	//获取动画的总时长
	var totalTime = choc.timeScroll.totalDuration();
	//获取到要到达的状态的时间
	var afterTime = choc.timeScroll.getLabelTime(step);
	//获取到滚动条能够滚动的最大的高度
	var maxH = $("body").height() - $(window).height();
	//计算出滚动条滚动的距离
	var positionY = afterTime/totalTime * maxH;
	//滚动条滚动的距离的持续时间
	var d = Math.abs( choc.timeScroll.time() - afterTime );

	var scrollAnimate = new TimelineMax();

	scrollAnimate.to("html,body",d,{scrollTop:positionY});

	choc.currentStep = step;
}
...
//在滚动条滚动的过程中，计算页面中应该到哪一个时间点上去
choc.scrollStatus = function (){
	var times = choc.scale() * choc.timeScroll.totalDuration();
	//当滚动条在滚动的过程中，让页面中的动画到打某个时间点
	choc.timeScroll.seek(times,false);
}

//切换整屏并且计算滚动条的距离

choc.changeStep = function(value){
	if( value === "next" ){ //向下切换

		//获取当前的时间
		var currentTime = choc.timeScroll.getLabelTime(choc.currentStep);

		//获取到下一个状态的字符串
		var afterStep = choc.timeScroll.getLabelAfter(currentTime);
		...
		var totalTime = choc.timeScroll.totalDuration();
		//获取到下一个状态的时间
		var afterTime = choc.timeScroll.getLabelTime(afterStep);
		//获取到滚动条能够滚动的最大的高度
		var maxH = $("body").height() - $(window).height();
		...
		//choc.timeScroll.tweenTo(afterStep);
		//记录当前的状态为下一个状态，方便继续切换到下一个状态上
		choc.currentStep = afterStep;
		...
		//获取到上一个状态的字符串
		var beforeStep = choc.timeScroll.getLabelBefore(currentTime);
		...
		var totalTime = choc.timeScroll.totalDuration();
		//获取到下一个状态的时间
		var BeforeTime = choc.timeScroll.getLabelTime(beforeStep);
		//获取到滚动条能够滚动的最大的高度
		var maxH = $("body").height() - $(window).height();
		...
		//滚动条滚动的距离的持续时间
		var d = Math.abs( choc.timeScroll.time() - BeforeTime );
		...
		//choc.timeScroll.tweenTo(beforeStep);
		//记录当前的状态为上一个状态，方便继续切换到上一个状态上
		choc.currentStep = beforeStep;
	}
}

//配置整屏切换的动画以及每一屏中的小动画

choc.configTimeScroll = function(){

	var time = choc.timeScroll ? choc.timeScroll.time() : 0;

	if( choc.timeScroll ) choc.timeScroll.clear();

	choc.timeScroll = new TimelineMax();

		// 当从第二屏切换到第一屏的时候，让第二屏里面的动画时间点重归0
		choc.timeScroll.to(".scene1",0,{onReverseComplete:function(){
			twoAnimate.timeline.seek(0,false);
		}},0);

		choc.timeScroll.to(".footer",0,{top:"100%"});

		choc.timeScroll.add("step1");
	choc.timeScroll.to(".scene2",0.8,{top:0,ease:Cubic.easeInOut});
	choc.timeScroll.to({},0.1,{onComplete:function(){
		menu.changeMenu("menu_state2");  //切换到第二屏调用的函数，同时传入导航条背景颜色变化的class名字
	},onReverseComplete:function(){
		menu.changeMenu("menu_state1"); 
	}},"-=0.2");
	//当切换到第二屏上的时候，翻转第二屏上的第一个动画
	choc.timeScroll.to({},0,{onComplete:function(){
		twoAnimate.timeline.tweenTo("state1");
	}},"-=0.2");

		choc.timeScroll.add("step2");
	...
	choc.timeScroll.to({},0.4,{});

		choc.timeScroll.add("point1");
	...
	choc.timeScroll.to({},0.4,{});
		...
		choc.timeScroll.add("point3");

	// --- 主动画中配置第二萍的小动画 end


	choc.timeScroll.to(".scene3",0.8,{top:0,ease:Cubic.easeInOut,onReverseComplete:function(){
	...
	choc.timeScroll.to({},0.4,{});
		...
		choc.timeScroll.add("threeSate");

	// --- 主动画中配置第三屏的小动画 end



	choc.timeScroll.to(".scene4",0.8,{top:0,ease:Cubic.easeInOut});
		choc.timeScroll.add("step4");

	//滚动到第五屏的时候，要让第四屏滚出屏幕外
	choc.timeScroll.to(".scene4",0.8,{top:-$(window).height(),ease:Cubic.easeInOut});
	//当可视区域大于950，那么就让导航条隐藏起来
	if($(window).width()>950){
		choc.timeScroll.to(".menu_wrapper",0.8,{top:-110,ease:Cubic.easeInOut},"-=0.8");
	}else{
		$(".menu_wrapper").css("top",0);
	}

	choc.timeScroll.to(".scene5",0.8,{top:0,ease:Cubic.easeInOut,onReverseComplete:function(){
		fiveAnimate.timeline.seek(0,false);
	}},"-=0.8");
	choc.timeScroll.to({},0.1,{onComplete:function(){
		fiveAnimate.timeline.tweenTo("fiveState");
	}},"-=0.2");
		choc.timeScroll.add("step5");

	choc.timeScroll.to(".scene5",0.5,{top:-$(".footer").height(),ease:Cubic.easeInOut});
	choc.timeScroll.to(".footer",0.5,{top:$(window).height()-$(".footer").height(),ease:Cubic.easeInOut},"-=0.5");

		choc.timeScroll.add("footer");

	choc.timeScroll.stop();
	//当改变浏览器的大小时，让动画走到之前已经到达的时间点
	choc.timeScroll.seek(time);
}

//配置导航条的动画
choc.configIntAnimate = function(){
...
// 3D翻转效果
choc.button3D = function(obj,element1,element2,d){
...
choc.imgWidth = function(elementImg){
